...
import { Field, FormProvider, FormConsumer } from '../components/my-formily/react'
...
const createPasswordEqualValidate = (equalName: string) => (field: any) => {
  if (form.values.confirm_password && field.value && form.values[equalName] !== field.value) {
...
const MyForm: React.FC = () => {
  return (
    <FormProvider form={form}>
      <Field name='name' title='Name' required initialValue='' decorator={[FormItem]} component={[Input, { placeholder: 'Please Input name' }]} />
      <Field
        name='password'
        title='Password'
        required
        initialValue=''
        decorator={[FormItem]}
        component={[Input, { placeholder: 'Please Input password~', type: 'password' }]}
